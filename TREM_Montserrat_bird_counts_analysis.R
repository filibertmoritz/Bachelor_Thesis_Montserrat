...
# best submodel for p(): ~day+time+rain+wind+activity+ridge, AIC difference to second best 12.09 - go on with this 
...
# best sub-model for psi(): ~alt, AIC difference to second/third best is 1.71 (~alt+treeheiht/alt+dbh) and 4. best is 4.29 (treeheight) - go on with this best one
...
fm17 <- colext(~alt, ~1, ~1, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm18 <- colext(~alt, ~alt, ~1, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm19 <- colext(~alt, ~1, ~alt, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm20 <- colext(~alt, ~alt, ~alt, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm21 <- colext(~alt, ~year, ~year, ~day+time+rain+wind+activity+ridge, data = umf, se = T)  ## this model will exclude the possibility that observed changes are just annual changes
...
# best model: year (fm21) has lowest AIC, with difference in AIC of 4.08 units to expansion model - this means that I will now check again with year/alt
...
print(g_e_modSel <- modSel(g_e_fitList))
# best model: still year model (fm21) with lowest AIC, second best expansion model with 4.08 AIC difference, constant model with 4.95 AIC difference - continue with year model (fm21)
...
##### 6: Goodness-of-Fit Test - MacKenzie-Bailey Goodness-of-Fit Test by function mb.gof.test() ####

# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## c-hat = 1.2, p-value = 0.079

# save output as Rdata
saveRDS(best_model_gof, file = sprintf('output/data/GOF/%s_gof_mb.rds', SPECIES))

##### 7: view results and extract data from best single model ####
...
# export everything backTransformed - but without SE!
...
# create input data.frame with values for the prediction, its not needed because of year as predictor in both: ext and col
...
#  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Brown Trembler') + # change Name
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Brown Trembler') +
