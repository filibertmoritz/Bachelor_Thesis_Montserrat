...
obsCov <- obsCov %>% # transfer wind data from data base (obsCov_db) to prepared data.frame
  ...
  select(-wind) %>% 
  rename(wind = Wind)
obsCov$wind[c(which(is.na(obsCov$wind)))] <- as.factor(1) # replace all the NA values by 1 for 'calm' - this is not really good, but I can't find another workaround
...
siteCovsColext <- siteCov %>% mutate(point = as.numeric(point), ridge = as.factor(ridge)) %>% 
  arrange(point) # sort rows in order of point number
...
# best submodel for p(): ~day+time+rain+wind+activity+ridge, AIC difference to second best 1.14 - go on with this 
...
# best sub-model for psi(): ~alt+treeheight, AIC difference to second best is 0.22 (~treeheight+dbh) - go on with this best one
...
# best model: expansion model (fm18) has lowest AIC, but not far away is the constant model with delta AIC 0.086
