...
yearlySiteCovsYear <- COUNTDATA %>% filter(species == SPECIES) %>% 
  group_by(point,year) %>%
  summarise(occu = as.numeric(mean(year) - 2010)) %>% # # code as.numeric
...
# organize models in fitList before printing the modSel-table
...
# best submodel for p(): ~day+time+rain+wind+activity, AIC difference to second best 1.68 - go on with this best
...
psi_fitList <- fitList('psi(.)g(.)e(.)p(~day+time+rain+wind+activity)' = fm6, 
                       'psi(~alt)g(.)e(.)p(~day+time+rain+wind+activity)' = fm8, 
                       'psi(~treeheight)g(.)e(.)p(~day+time+rain+wind+activity)' = fm9, 
                       'psi(~alt+treeheight)g(.)e(.)p(~day+time+rain+wind+activity)' = fm10,
                       'psi(~dbh)g(.)e(.)p(~day+time+rain+wind+activity)' = fm11, 
                       'psi(alt+dbh)g(.)e(.)p(~day+time+rain+wind+activity)' = fm12, 
                       'psi(treeheight+dbh)g(.)e(.)p(~day+time+rain+wind+activity)' = fm13, 
                       'psi(~alt+treeheight+dbh)g(.)e(.)p(~day+time+rain+wind+activity)' = fm14, 
                       'psi(alt:treeheight)g(.)e(.)p(~day+time+rain+wind+activity)' = fm15, 
                       'psi(~alt:treeheight+dbh)g(.)e(.)p(~day+time+rain+wind+activity)' = fm16)
print(psi_modSel <- modSel(psi_fitList)) 
# best sub-model for psi(): ~alt+treeheight, AIC difference to second best is 1.94 (~alt+treeheight+dbh) and third best is 2.45 (~treeheight) - go on with the AIC best one
...
fm17 <- colext(~alt+treeheight, ~1, ~1, ~day+time+rain+wind+activity, data = umf, se = T)
fm18 <- colext(~alt+treeheight, ~alt, ~1, ~day+time+rain+wind+activity, data = umf, se = T)
fm19 <- colext(~alt+treeheight, ~1, ~alt, ~day+time+rain+wind+activity, data = umf, se = T)
fm20 <- colext(~alt+treeheight, ~alt, ~alt, ~day+time+rain+wind+activity, data = umf, se = T)
fm21 <- colext(~alt+treeheight, ~year, ~year, ~day+time+rain+wind+activity, data = umf, se = T)  ## this model will exclude the possibility that observed changes are just annual changes
...
# best model: year model (fm21) has lowest AIC, second best constant model with 40.77 AIC difference, contraction model with 42.3 AIC difference - this means that I will now have to check after correcting for yearly variation again for contraction, expansion and shift
...
print(g_e_modSel <- modSel(g_e_fitList))
# best model: still year model (fm21) with lowest AIC, second best year_contraction model with 1.04 AIC difference, year_expansion model with 1.16 AIC difference - continue with year model (fm21)

###### 4.5: export all required model selection tables ####
...
# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## p-value of 0.46, c-hat estimate of 1
...
###### 7.1 plot relationship between colonization and extinction with altitude, NOT DONE FOR THIS SPECIES BECAUSE ALT IS NOT IN NEEDED SUBMODELS #####
...
# predict values by input data from nd, add column with rescaled altitude - not done for CAEL because there is no effect of alt
...
#  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Caribbean Elaenia') + # change Name
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Caribbean Elaenia') +
