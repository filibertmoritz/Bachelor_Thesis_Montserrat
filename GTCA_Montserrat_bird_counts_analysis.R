...
# best submodel for p(): ~day+time+rain+wind+activity, AIC difference to second best 1.19 - go on with this best
...
# best sub-model for psi(): ~treeheight, AIC difference to second best is 0.61 (~1) and third best is 1.92 (dbh) - go on with this best one
...
fm17 <- colext(~treeheight, ~1, ~1, ~day+time+rain+wind+activity, data = umf, se = T)
fm18 <- colext(~treeheight, ~alt, ~1, ~day+time+rain+wind+activity, data = umf, se = T)
fm19 <- colext(~treeheight, ~1, ~alt, ~day+time+rain+wind+activity, data = umf, se = T)
fm20 <- colext(~treeheight, ~alt, ~alt, ~day+time+rain+wind+activity, data = umf, se = T)
fm21 <- colext(~treeheight, ~year, ~year, ~day+time+rain+wind+activity, data = umf, se = T)  ## this model will exclude the possibility that observed changes are just annual changes
...
# best model: year model (fm21) has lowest AIC, second best constant model with 30.56 AIC difference, expansion model with 32.08 AIC difference - go on with single best, but after correcting für yearly variation test again for range shift
...
print(g_e_modSel <- modSel(g_e_fitList))
# best model: still year model (fm21) with lowest AIC, second best year_expansion model with 0.41 AIC difference, year_contraction model with 0.66 AIC difference - continue with year model (fm21)
...
##### 5: Goodness-of-Fit Test MacKenzie-Bairley Goodness-of-Fit Test by function mb.gof.test() ####

# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## c-hat 1.07 and p-value = 0.275
...
# save in german for plotting 
#pred_ext_col_ger <- pred_ext_col %>% 
#  mutate(Typ = ifelse(Type == 'Extinction', 'Extinktion', 'Besiedlung')) %>% 
#  select(-Type)
...
#  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Green-Throated Carib') + # change Name
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Green-Throated Carib') +
