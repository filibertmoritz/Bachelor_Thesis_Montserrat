...
# best submodel for p(): ~day+time+rain+wind+activity+ridge, AIC difference to second best 0.11 - go on with this 
...
# best sub-model for psi(): ~treeheight, AIC difference to second best is 0.19 (~treeheight+dbh) - go on with this best one
...
# best model: expansion model (fm18) has lowest AIC with little difference to the second best (shift) model (delta AIC: 0.93) and third best (contraction) with delta AIC 1.63 units
...
# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## p-value of 0, c-hat estimate 2.11
...
pred_ext_col_plot <- pred_col %>%
  ...
  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Bridled Quail−Dove') + # change Name
