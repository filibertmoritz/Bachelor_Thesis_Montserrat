...
# best submodel for p(): ~day+time+rain+wind+activity+ridge, AIC difference to second best 13.68 - go on with this 
...
# best sub-model for psi(): ~1, AIC difference to second best is 2 (~treeheight/alt:treeheight/dbh) - go on with this best one
...
# best model: contraction model (fm19) has lowest AIC, with 1,82 AIC units difference to shift model - these two are far away from constant model
...
# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## p-value of 0, c-hat estimate at 2.13
...
# predict values by input data from nd, add column with rescaled altitude - here only extinction, because colonization wasn't explained by alt
...
pred_ext_plot <- pred_ext %>%
  ...
  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Forest Thrush') + # change Name
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Forest Thrush') +
