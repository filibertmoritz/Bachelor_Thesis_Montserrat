...
# best submodel for p(): ~day+time+rain+wind+activity, AIC difference to second best 1.98 - go on with this best
...
# best sub-model for psi(): ~1, AIC difference to second best is 0.43 (~alt+treeheight) and third best is 0.44 (~alt+dbh) - go on with this best one
...
fm17 <- colext(~1, ~1, ~1, ~day+time+rain+wind+activity, data = umf, se = T)
fm18 <- colext(~1, ~alt, ~1, ~day+time+rain+wind+activity, data = umf, se = T)
fm19 <- colext(~1, ~1, ~alt, ~day+time+rain+wind+activity, data = umf, se = T)
fm20 <- colext(~1, ~alt, ~alt, ~day+time+rain+wind+activity, data = umf, se = T)
fm21 <- colext(~1, ~year, ~year, ~day+time+rain+wind+activity, data = umf, se = T)  ## this model will exclude the possibility that observed changes are just annual changes
...
# best model: contraction model (19) has lowest AIC, second best shift model with 3.61 AIC difference - go on with single best
...
best_model <- fm19 # fill in best model
...
# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## p-value of 0, c-hat estimate of 2.46
...
###### 7.1 plot relationship between colonization and extinction with altitude  - NOT NEEDED BECAUSE ALT IS NOT INCLUDED IN BEST MODEL! #####
...
# predict values by input data from nd, add column with rescaled altitude, only predict extinction (col is constant)
...
fwrite(pred_ext, file = sprintf('output/data/pred_col_ext/%s_pred_ext_col.csv', SPECIES))

# plot colonization~altitude and extinction~altitude together
pred_ext_col_plot <- pred_ext %>%
  ggplot(aes(x = Altitude, y = Predicted, colour = Type, fill = Type)) +
  geom_line(linewidth = 3) +
  geom_ribbon(aes(ymin = lower,ymax = upper), alpha = 0.2) +
  
  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Scaly-Naped Pigeon') + # change Name
  scale_fill_viridis_d(alpha=0.3,begin=0.98,end=0.98,direction=1) + # set begin/end to 0.98 for yellow/ext
  scale_color_viridis_d(alpha=1,begin=0.98,end=0.98,direction=1) +
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Scaly-Naped Pigeon') +
