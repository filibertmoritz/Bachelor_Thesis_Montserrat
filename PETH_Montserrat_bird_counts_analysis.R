...
# best submodel for p(): ~day+time+rain+wind+activity+ridge, AIC difference to second best 1.33 - go on with this 
...
# best sub-model for psi(): ~1, AIC difference to second best is 1.99 (~alt) and third best is 1.99 (alt:treeheight) - go on with this best one
...
# best model: contraction model (fm19) has lowest AIC, second best constant model with 1.28 AIC difference, shift model with 2.00 AIC difference - go on with single best
...
# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## p-value of 0, c-hat estimate of 1.45
...
###### 7.1 plot relationship between colonization and extinction with altitude NOTE: ONLY EXT BECAUSE OF BEST MODEL#####
...
pred_ext_ger <- pred_ext %>% 
  mutate(Typ = ifelse(Type == 'Extinction', 'Extinktion', 'Besiedlung')) %>% 
  select(-Type)

# plot colonization~altitude and extinction~altitude together
pred_ext_plot <- pred_ext %>%
  ggplot(aes(x = Altitude, y = Predicted, colour = Type, fill = Type)) +
  geom_line(linewidth = 3) +
  geom_ribbon(aes(ymin = lower,ymax = upper), alpha = 0.2) +
  
  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Pearly-Eyed Thrasher') + # change Name
  scale_fill_viridis_d(alpha=0.3,begin=0.98,end=0.98,direction=1) + # set begin/end to 0.98 for yellow/ext // 0 for purple/col
...
saveRDS(pred_ext_plot, file = sprintf('output/plot/data/%s_ext_col_plot.rds', SPECIES))
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Pearly-Eyed Thrasher') +
