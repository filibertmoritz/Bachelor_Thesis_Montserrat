...
# best submodel for p(): ~day+time+rain+wind+activity+ridge, AIC difference to second best 15.19 - go on with this 
...
# best sub-model for psi(): ~treeheight, AIC difference to second best is 0.89 (~alt+treeheight) and third best is 1.53 (treeheight+dbh) - go on with this best one
...
fm17 <- colext(~treeheight, ~1, ~1, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm18 <- colext(~treeheight, ~alt, ~1, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm19 <- colext(~treeheight, ~1, ~alt, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm20 <- colext(~treeheight, ~alt, ~alt, ~day+time+rain+wind+activity+ridge, data = umf, se = T)
fm21 <- colext(~treeheight, ~year, ~year, ~day+time+rain+wind+activity+ridge, data = umf, se = T)  ## this model will exclude the possibility that observed changes are just annual changes
...
# best model: expansion model (fm18) has lowest AIC, second best constant model with 1.36 AIC difference, shift model with 1.96 AIC difference - go on with single best
...
best_model <- fm18 # fill in best model
...
# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## p-value of 0, c-hat estimate of 1.81
...
###### 7.1 plot relationship between colonization and extinction with altitude NOTE: ONLY COL BECAUSE OF BEST MODEL!!! #####
...
fwrite(pred_col, file = sprintf('output/data/pred_col_ext/%s_pred_ext_col.csv', SPECIES))

# plot colonization~altitude and extinction~altitude together
pred_col_plot <- pred_col %>%
  ggplot(aes(x = Altitude, y = Predicted, colour = Type, fill = Type)) +
  geom_line(linewidth = 3) +
  geom_ribbon(aes(ymin = lower,ymax = upper), alpha = 0.2) +
  
  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Purple-Throated Carib') + # change Name
  scale_fill_viridis_d(alpha=0.3,begin=0,end=0,direction=1) + # set begin/end to 0.98 for yellow/ext // 0 for purple/col
  scale_color_viridis_d(alpha=1,begin=0,end=0,direction=1) +
...
saveRDS(pred_col_plot, file = sprintf('output/plot/data/%s_ext_col_plot.rds', SPECIES))
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Purple-Throated Carib') +
