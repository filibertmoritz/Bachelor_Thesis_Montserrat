...
# set working directory and load prepared data from Steffen # for Filibert
...
load(file = 'data/MONTSERRAT_ANNUAL_DATA_INPUT2023.RData') # for Filibert
...
###### 2.4: rename variate names/columns #####
...
yearlySiteCovsYear <- COUNTDATA %>% filter(species == SPECIES) %>% 
  group_by(point,year) %>%
  summarise(occu = as.numeric(mean(year) - 2010)) %>% # code year as.numeric
...
str(umf) # looks good, pay attention: not all variables from this data set are coded in an appropriate way (as.factor())
...
# best submodel for p(): ~day+time+rain+wind+activity+ridge, AIC difference to second best 4.22 - go on with this 
...
# best sub-model for psi(): ~alt:treeheight, AIC difference to second best is 1.77 (~alt:treeheight+dbh) - go on with this first best one
...
# best model: shift model (fm20) has lowest AIC, second best is 0.89 AIC units apart (expansion)
...
##### 5: MacKenzie-Bailey Goodness-of-Fit Test by function mb.gof.test() ####

# calculate GoF-Test
(best_model_gof <- mb.gof.test(best_model, nsim = 1000)) ## p-value of 0.062 is ok, c-hat 1.2
...
# extract everything (psi, col, ext, det) - on logit-scale!
...
# export everything transformed to original scale
...
pred_ext_col_plot <- pred_ext_col %>%
  ...
  labs(x="Elevation (m above sea level)", y="Predicted probability", title='Montserrat Oriole') + # change Name
...
occupancy_plot <- ggplot(data = occupancy_data, aes(x = Year, y = Occupancy)) +
  ...
  labs(x="Year", y="Mean occupancy probability", title='Montserrat Oriole') +
